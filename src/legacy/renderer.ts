...
interface EvacPlanState {
  roadDensities?: Array<{ edge: { from: number; to: number }; density: number }>;
  graph?: { nodes: Array<{ x: number; y: number }> };
  cityPlans?: Array<{
...
interface RendererState extends AppState {
  mapCtx?: CanvasRenderingContext2D;
  animationId?: number;
  selectedShelterIndex?: number;
  evacuationPlan?: EvacPlanState;
}
...
(function (global: Window & typeof globalThis): void {
    ...
    function getZoneColors(key: string, tintIndex: number): ZoneColors {
    ...
    function getTerrainColor(elevation: number): TerrainColor {
        if (elevation <= -100) {
    ...
    function drawTerrainHeatmap(
        ...
        for (let y: number = 0; y < height; y += step) {
            for (let x: number = 0; x < width; x += step) {
                ...
                const color: TerrainColor = getTerrainColor(elev);
        ...
        const FEATURE_TYPES: Record<string, string> = global.Physics.TERRAIN_FEATURE_TYPES;
        ...
        contourLevels.forEach(function (level: number, levelIdx: number): void {
            ...
            for (let y: number = step; y < height - step; y += step * 2) {
                for (let x: number = step; x < width - step; x += step * 2) {
                    ...
                    pairs.forEach(function (pair: [number, number, number, number, number, number]): void {
    ...
    function drawPolygonPathFromPoints(
        ctx: CanvasRenderingContext2D,
        points: Point[] | TerrainPolygonPoint[],
        close: boolean
    ): void {
        ...
        for (let i: number = 1; i < points.length; i++) {
            const prev: Point | TerrainPolygonPoint = points[i - 1];
            const curr: Point | TerrainPolygonPoint = points[i];
            const midX: number = (prev.x + curr.x) / 2;
            const midY: number = (prev.y + curr.y) / 2;
            ctx.quadraticCurveTo(prev.x, prev.y, midX, midY);
        }
        if (close !== false) {
            const last: Point | TerrainPolygonPoint = points[points.length - 1];
            const first: Point | TerrainPolygonPoint = points[0];
    ...
    function drawRoadsLayer(
        ...
        const roads: any[] = global.Physics.generateRoads(width, height, cities);
        mapCtx.strokeStyle = rgba(150, 140, 100, 0.25);
        mapCtx.lineWidth = 2;
        mapCtx.setLineDash([]);
        roads.forEach(function (road: any): void {
            mapCtx.beginPath();
    ...
    function drawEvacuationRoads(mapCtx: CanvasRenderingContext2D, state: RendererState): void {
        const roadDensities: Array<{ edge: { from: number; to: number }; density: number }> = state.evacuationPlan!.roadDensities!;
        const nodes: Array<{ x: number; y: number }> = state.evacuationPlan!.graph!.nodes;

        roadDensities.forEach(function (rd: { edge: { from: number; to: number }; density: number }): void {
            const edge: { from: number; to: number } = rd.edge;
            const density: number = rd.density;

            const fromNode: { x: number; y: number } | undefined = nodes[edge.from];
            const toNode: { x: number; y: number } | undefined = nodes[edge.to];
    ...
    function drawEvacuationArrows(mapCtx: CanvasRenderingContext2D, state: RendererState): void {
        const cityPlans: Array<{
            path?: Array<{ from: number; to: number }>;
            evacuated?: number;
            canEvacuate?: boolean;
        }> = state.evacuationPlan!.cityPlans!;
        const nodes: Array<{ x: number; y: number }> = state.evacuationPlan!.graph!.nodes;
        ...
        cityPlans.forEach(function (plan: {
            ...
            if (!plan.path || plan.path.length === 0 || (plan.evacuated ?? 0) <= 0) return;

            plan.path.forEach(function (edge: { from: number; to: number }): void {
                const fromNode: { x: number; y: number } | undefined = nodes[edge.from];
                const toNode: { x: number; y: number } | undefined = nodes[edge.to];
                if (!fromNode || !toNode) return;
                ...
                const arrowAlpha: number = plan.canEvacuate ? 0.7 : 0.4;
    ...
    function drawSheltersOnMap(mapCtx: CanvasRenderingContext2D, state: RendererState): void {
    ...
    function drawCities(mapCtx: CanvasRenderingContext2D, state: RendererState): void {
        const BUILDING_TYPES: Record<string, any> = global.Physics.BUILDING_TYPES;
        ...
        state.cities.forEach(function (city: City, index: number): void {
            ...
            if (city.buildingDistribution && size > 18) {
                ...
                BUILDING_TYPE_ORDER.forEach(function (type: string): void {
                    const ratio: number = (city.buildingDistribution as any)[type] || 0;
    ...
    function getOverlayHatchColors(): Record<string, OverlayHatchStyle> {
        const zones: ZoneDef[] = getZoneDefs();
        const result: Record<string, OverlayHatchStyle> = {};
    ...
    function generateCombinations<T>(arr: T[], k: number): T[][] {
        ...
        while (true) {
            result.push(idx.map(function (i: number): T { return arr[i]; }));
            let pos: number = k - 1;
    ...
    function drawMultiCircleOverlap(
        ctx: CanvasRenderingContext2D,
        circles: CircleR[],
        zoneIndex: number,
        level: number
        ...
        const lvlStyle: OverlapLevelStyle | null | undefined = OVERLAP_LEVEL_STYLES[level];
        if (!lvlStyle) {
            ctx.restore();
            return;
        }
        ...
        if (lvlStyle.hatch2) {
            ...
            const overlayHatchColors: Record<string, OverlayHatchStyle> = getOverlayHatchColors();
            const baseStyle: OverlayHatchStyle = overlayHatchColors[zoneDefs[zoneIndex].key];
            ...
            const sp2: number = lvlStyle.spacing2 || 6;
        ...
        if (lvlStyle.outline) {
            const color: string = lvlStyle.outlineColor || 'rgba(255, 255, 255, 0.55)';
    ...
    function drawHatchInsidePath(ctx: CanvasRenderingContext2D, style: OverlayHatchStyle): void {
        ctx.save();
        ctx.clip();
    ...
    function drawAllOverlapHighlights(mapCtx: CanvasRenderingContext2D, state: RendererState): void {
        const scaled: ScaledExplosion[] = [];
        ...
        zoneDefs.forEach(function (zone: ZoneDef, zoneIndex: number): void {
            ...
            const baseStyle: OverlayHatchStyle = overlayHatchColors[zk];

            for (let level: number = 2; level <= maxLevel; level++) {
                const combos: ScaledExplosion[][] = generateCombinations(scaled, level);
                for (let ci: number = 0; ci < combos.length; ci++) {
                    const combo: ScaledExplosion[] = combos[ci];
                    const circlesWithR: CircleR[] = [];
                    ...
                    if (level === 2) {
                        const A: CircleR = circlesWithR[0];
                        const B: CircleR = circlesWithR[1];
    ...
    function drawOneExplosionZones(
        ...
        const boundaryCache: Record<string, TerrainPolygonPoint[]> = {};
        if (useTerrain) {
        ...
        zoneDefs.forEach(function (zone: ZoneDef): void {
            const colors: ZoneColors = getZoneColors(zone.key, tintIndex);
            ...
            if (useTerrain && boundaryCache[zone.key]) {
                const points: TerrainPolygonPoint[] = boundaryCache[zone.key];
            ...
            if (state.showLabels && pxRadius > 30 && isSelected) {
                ...
                if (useTerrain && boundaryCache[zone.key]) {
                    ...
                    const pt: TerrainPolygonPoint = points[closestIdx];

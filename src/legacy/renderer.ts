...
const rgba: (r: number, g: number, b: number, a: number) => string = DataDisplay.rgba

function getZoneDefs(): ZoneDef[] {
  return window.Physics.getZones()
}
...
function setupCanvas(
  ...
  state.cities = window.Physics.generateCities(width, height)
...
function drawTerrainHeatmap(
  ...
  for (let y: number = 0; y < height; y += step) {
    for (let x: number = 0; x < width; x += step) {
      const elev: number = window.Physics.getElevationAt(x + step / 2, y + step / 2, terrain)
  ...
  const FEATURE_TYPES: Record<string, string> = window.Physics.TERRAIN_FEATURE_TYPES
  ...
  contourLevels.forEach(function (level: number, levelIdx: number): void {
    ...
    for (let y: number = step; y < height - step; y += step * 2) {
      for (let x: number = step; x < width - step; x += step * 2) {
        const e0: number = window.Physics.getElevationAt(x, y, terrain)
        const e1: number = window.Physics.getElevationAt(x + step, y, terrain)
        const e2: number = window.Physics.getElevationAt(x + step, y + step, terrain)
        const e3: number = window.Physics.getElevationAt(x, y + step, terrain)
...
function drawRoadsLayer(
  ...
  const roads: any[] = window.Physics.generateRoads(width, height, cities)
...
function drawEvacuationRoads(
  mapCtx: CanvasRenderingContext2D,
  state: AppState
): void {
  const roadDensities: any[] = state.evacuationPlan.roadDensities
  const nodes: any[] = state.evacuationPlan.graph.nodes

  roadDensities.forEach(function (rd: any): void {
    const edge: any = rd.edge
    const density: number = rd.density

    const fromNode: any = nodes[edge.from]
    const toNode: any = nodes[edge.to]
...
function drawEvacuationArrows(
  mapCtx: CanvasRenderingContext2D,
  state: AppState
): void {
  const cityPlans: any[] = state.evacuationPlan.cityPlans
  const nodes: any[] = state.evacuationPlan.graph.nodes

  cityPlans.forEach(function (plan: any): void {
    if (!plan.path || plan.path.length === 0 || plan.evacuated <= 0) return

    plan.path.forEach(function (edge: any): void {
      const fromNode: any = nodes[edge.from]
      const toNode: any = nodes[edge.to]
      if (!fromNode || !toNode) return
...
function drawSheltersOnMap(
  mapCtx: CanvasRenderingContext2D,
  state: AppState
): void {
  if (!state.shelters) return
  state.shelters.forEach(function (shelter: Shelter, idx: number): void {
    ...
    if (state.showLabels) {
      ...
      const label: string = shelter.name
...
function drawMultiCircleOverlap(
  ...
  const lvlStyle: OverlapLevelStyle | null = OVERLAP_LEVEL_STYLES[level]
  if (!lvlStyle) {
    ctx.restore()
    return
  }
...
function buildCircleIntersectionPath(
  ...
  const p1x: number = px + ix
  const p1y: number = py + iy
  const p2x: number = px - ix
  const p2y: number = py - iy
...
function drawHatchInsidePath(
  ctx: CanvasRenderingContext2D,
  style: OverlayHatchStyle
): void {
  ctx.save()
  ctx.clip()
...
function drawAllOverlapHighlights(
  mapCtx: CanvasRenderingContext2D,
  state: AppState
): void {
  ...
  zoneDefs.forEach(function (zone: ZoneDef, zoneIndex: number): void {
    ...
    for (let level: number = 2; level <= maxLevel; level++) {
      ...
      for (let ci: number = 0; ci < combos.length; ci++) {
        ...
        if (level === 2) {
          const A: CircleData = circlesWithR[0]
          const B: CircleData = circlesWithR[1]
...
function drawOneExplosionZones(
  ...
  if (useTerrain && terrain) {
    zoneDefs.forEach(function (zone: ZoneDef): void {
      boundaryCache[zone.key] = window.Physics.generateTerrainBoundaryPolygon(
  ...
  zoneDefs.forEach(function (zone: ZoneDef): void {
    ...
    const zoneBoundary = boundaryCache[zone.key]
    if (useTerrain && zoneBoundary) {
      const points: TerrainBoundaryPoint[] = zoneBoundary
    ...
    if (state.showLabels && pxRadius > 30 && isSelected) {
      ...
      const zoneBoundary2 = boundaryCache[zone.key]
      if (useTerrain && zoneBoundary2) {
        const points: TerrainBoundaryPoint[] = zoneBoundary2

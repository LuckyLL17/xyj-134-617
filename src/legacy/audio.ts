import type { AudioManagerModule } from '@/types/index'

export interface ExplosionSoundLayerOptions {
...
export interface ExplosionSoundLayerResult {
  osc: OscillatorNode
  gain: GainNode
...
export interface SoundResult {
  ctx: AudioContext
  group: GainNode
  layers: (ExplosionSoundLayerResult | NoiseLayerResult)[]
  startTime: number
}

export interface ExplosionAudioSequence {
  flash: SoundResult | null
  explosion?: SoundResult | null
  shockwave?: SoundResult | null
  quake?: SoundResult | null
  debris?: SoundResult | null
}
...
function playExplosionSound(intensity: number): SoundResult | null {
  ...
  const layers: (ExplosionSoundLayerResult | NoiseLayerResult)[] = []
...
function playShockwaveSound(distance: number, intensity: number): SoundResult | null {
  const ctx = ensureAudio()
  if (!ctx) return null
...
function playQuakeSound(intensity: number, delay?: number): SoundResult | null {
  const ctx = ensureAudio()
  if (!ctx) return null
...
function playFlashSound(): SoundResult | null {
  const ctx = ensureAudio()
  if (!ctx) return null
...
function playDebrisSound(intensity: number): SoundResult | null {
  const ctx = ensureAudio()
  if (!ctx) return null
...
export const AudioManager = {
  ...
  playFlashSound,
  playDebrisSound,
  playExplosionAudioSequence,
  setMasterVolume
} as unknown as AudioManagerModule

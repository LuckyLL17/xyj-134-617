...
(function (global: Window) {
    ...
    function createExplosionSoundLayer(
        ...
        osc.type = type || 'sine';
        osc.frequency.setValueAtTime(frequencyStart || 0, startTime);
        osc.frequency.exponentialRampToValueAtTime(
            Math.max(0.1, frequencyEnd || 0),
            startTime + (duration || 0)
        );

        const peakTime = startTime + (attack || 0);
        gain.gain.setValueAtTime(0, startTime);
        gain.gain.linearRampToValueAtTime(startGain || 0, peakTime);
        gain.gain.setValueAtTime(startGain || 0, startTime + (duration || 0) - (release || 0));
        gain.gain.exponentialRampToValueAtTime(Math.max(0.001, endGain || 0), startTime + (duration || 0));
        ...
        gain.connect(dest);
        osc.start(startTime);
        osc.stop(startTime + (duration || 0) + 0.1);
    ...
    ): { noiseSource: AudioBufferSourceNode; gain: GainNode; filter: BiquadFilterNode } {
        const {
            duration,
            startGain,
            endGain,
    ...
    function playExplosionSound(intensity: number): {
        ctx: AudioContext;
        group: GainNode;
        layers: Array<{ osc: OscillatorNode; gain: GainNode; filter: BiquadFilterNode | null } | { noiseSource: AudioBufferSourceNode; gain: GainNode; filter: BiquadFilterNode }>;
        startTime: number;
    } | null {
        ...
        const layers: Array<{ osc: OscillatorNode; gain: GainNode; filter: BiquadFilterNode | null } | { noiseSource: AudioBufferSourceNode; gain: GainNode; filter: BiquadFilterNode }> = [];
    ...
    function playShockwaveSound(distance: number, intensity: number): {
        ctx: AudioContext;
        group: GainNode;
        layers: Array<{ osc: OscillatorNode; gain: GainNode; filter: BiquadFilterNode | null } | { noiseSource: AudioBufferSourceNode; gain: GainNode; filter: BiquadFilterNode }>;
        startTime: number;
    ...
    function playQuakeSound(intensity: number, delay?: number): {
        ctx: AudioContext;
        group: GainNode;
        layers: Array<{ osc: OscillatorNode; gain: GainNode; filter: BiquadFilterNode | null } | { noiseSource: AudioBufferSourceNode; gain: GainNode; filter: BiquadFilterNode }>;
        startTime: number;
    ...
    function playDebrisSound(intensity: number): {
        ctx: AudioContext;
        group: GainNode;
        layers: Array<{ osc: OscillatorNode; gain: GainNode; filter: BiquadFilterNode | null } | { noiseSource: AudioBufferSourceNode; gain: GainNode; filter: BiquadFilterNode }>;
        startTime: number;
    ...
    function playExplosionAudioSequence(intensity: number, maxRadius: number): ExplosionSounds {
        const sounds: ExplosionSounds = {};

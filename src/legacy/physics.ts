...
(function (global: Window) {
    ...
    const BOMB_TYPES: Record<BombTypeKey, BombType> = {
    ...
    const TERRAIN_FEATURE_TYPES: Record<string, TerrainFeatureType> = {
        MOUNTAIN: 'mountain',
        HILL: 'hill',
        BASIN: 'basin',
        PLAIN: 'plain'
    ...
    const TERRAIN_PRESETS: Record<TerrainPresetKey, TerrainPreset> = {
    ...
    const BUILDING_TYPES: Record<BuildingTypeKey, BuildingType> = {
        temporary: {
            name: '临时建筑',
            shortName: '临建',
            color: '#8b7355',
    ...
    const BUILDING_TYPE_ORDER: BuildingTypeKey[] = ['temporary', 'wood', 'brick', 'rc_frame', 'steel', 'rc_core', 'blast_resistant'];

    const DAMAGE_LEVELS: Record<DamageLevelKey, DamageLevel> = {
    ...
    function getBuildingDamageLevel(buildingType: BuildingTypeKey, overpressurePsi: number): DamageLevelKey {
    ...
    function calculateBuildingCasualties(
        population: number,
        ...
        const injuredRatioMap: Record<DamageLevelKey, number> = {
            intact: 0.02,
            light: 0.08,
            moderate: 0.2,
            severe: 0.35,
        ...
        const injuredRatio: number = injuredRatioMap[damageLevel] || 0;
    ...
    function getZoneAltitudeSensitivity(): Record<string, number> {
    ...
    function addZone(zoneDef: Partial<ZoneDef> & { key: string; label: string }): ZoneOperationResult {
    ...
    function updateZone(key: string, updates: Partial<ZoneDef>): ZoneOperationResult {
    ...
    function removeZone(key: string): ZoneOperationResult {
        const zoneIndex = ZONE_DEFS.findIndex(function (z) { return z.key === key; });
        if (zoneIndex < 0) {
            return { success: false, error: '圈层不存在' };
        }
    ...
    function resetZones(): ZoneOperationResult {
        ZONE_DEFS = JSON.parse(JSON.stringify(DEFAULT_ZONE_DEFS));
        return { success: true, zones: ZONE_DEFS };
    }

    function moveZone(key: string, newIndex: number): ZoneOperationResult {
        const zoneIndex = ZONE_DEFS.findIndex(function (z) { return z.key === key; });
        if (zoneIndex < 0) {
            return { success: false, error: '圈层不存在' };
        }
    ...
    function calculateRadii(yieldKilotons: number, burstHeight: number): Record<string, number> {
        const W_megatons = yieldKilotons / 1000;
        const result: Record<string, number> = {};
    ...
    function generateBuildingDistribution(isCentral: boolean, size: number): Record<BuildingTypeKey, number> {
        const dist: Partial<Record<BuildingTypeKey, number>> = {};
        ...
        BUILDING_TYPE_ORDER.forEach(function (type) {
            ...
            dist[type] = Math.max(0.01, (dist[type] || 0) + jitterAmount);
            total += dist[type] || 0;
        });

        BUILDING_TYPE_ORDER.forEach(function (type) {
            dist[type] = (dist[type] || 0) / total;
        });

        return dist as Record<BuildingTypeKey, number>;
    }
    ...
    function calculateCasualtiesForZone(pop: number, zoneName: string): CasualtyResult & { destroyed: boolean } {
    ...
    function calculateCityBuildingDamage(
        city: City,
        ...
        const buildingResults: Partial<Record<BuildingTypeKey, BuildingDamageResult>> = {};
        let totalDeaths = 0;
        let totalInjured = 0;
        let totalDestroyedPop = 0;
        let totalAffectedPop = 0;

        const distByDamage: Record<DamageLevelKey, number> = {
            intact: 0,
            light: 0,
            moderate: 0,
            severe: 0,
        ...
        return {
            ...
            buildingResults: buildingResults as Record<BuildingTypeKey, BuildingDamageResult>,
    ...
    function calculateAvgStructureFactor(buildingDistribution: Record<BuildingTypeKey, number>): number {
        let total = 0;
        let weightedSum = 0;
        BUILDING_TYPE_ORDER.forEach(function (type) {
    ...
    function calculateAllCitiesBuildingDamage(
        cities: City[],
        explosions: Explosion[],
        scale: number,
        terrain?: TerrainData | null
        ...
        const totalByDamage: Record<DamageLevelKey, number> = {
            intact: 0,
            light: 0,
            moderate: 0,
            severe: 0,
        ...
        const totalByBuildingType: Partial<Record<BuildingTypeKey, { population: number; deaths: number; injured: number }>> = {};
        BUILDING_TYPE_ORDER.forEach(function (type) {
        ...
        cities.forEach(function (city) {
            ...
            BUILDING_TYPE_ORDER.forEach(function (type) {
                if (result.buildingResults[type]) {
                    (totalByBuildingType[type] as { population: number; deaths: number; injured: number }).population += result.buildingResults[type].population;
                    (totalByBuildingType[type] as { population: number; deaths: number; injured: number }).deaths += result.buildingResults[type].deaths;
                    (totalByBuildingType[type] as { population: number; deaths: number; injured: number }).injured += result.buildingResults[type].injured;
                }
            });
        ...
        return {
            ...
            totalByBuildingType: totalByBuildingType as Record<BuildingTypeKey, { population: number; deaths: number; injured: number }>,
    ...
    function calculateAffectedArea(radii: Record<string, number>): number {
    ...
    function calculateCircleUnionAreaInclusion(circles: Circle[]): number {
    ...
    function generateTerrainFeatures(
        ...
        function addFeature(type: TerrainFeatureType, x: number, y: number, radius: number, heightVal: number): void {
            features.push({
                type: type,
                x: x,
                y: y,
    ...
    function getElevationAt(x: number, y: number, terrain: TerrainData | null): number {
    ...
    function generateTerrainBoundaryPolygon(
        explosion: Explosion,
        zoneName: string,
        terrain: TerrainData | null,
        scale: number,
        segments?: number
    ): TerrainPolygonPoint[] | null {
        ...
        const points: TerrainPolygonPoint[] = [];
    ...
    function checkPointInAnyZoneTerrainAware(
        point: Point,
        explosions: Explosion[],
        terrain: TerrainData | null,
        scale: number
    ...
    function getWorstZoneForCityTerrain(
        city: City,
        explosions: Explosion[],
        scale: number,
        terrain: TerrainData | null
    ...
    function calculateCasualtiesTerrainAware(
        cities: City[],
        explosions: Explosion[],
        scale: number,
        terrain: TerrainData | null
    ...
    function calculateShockwaveRadiusAtAngle(
        explosion: Explosion,
        angle: number,
        zoneKey: string,
        terrain: TerrainData | null,
    ...
    function buildEvacuationGraph(
        cities: City[],
        shelters: Shelter[],
        roads: Road[],
        capacityMultiplier?: number
    ): { nodes: EvacNode[]; nodeMap: Record<string, number> } {
    ...
    function dijkstra(
        graph: { nodes: EvacNode[]; nodeMap: Record<string, number> },
        sourceId: number
    ): { dist: Record<number, number>; prev: Record<number, { from: number; edge: EvacEdge } | null> } {
        const dist: Record<number, number> = {};
        const prev: Record<number, { from: number; edge: EvacEdge } | null> = {};
        ...
        while (true) {
            ...
            currentNode.edges.forEach(function (edge) {
                ...
                const alt = dist[currentId as number] + edge.lengthPx;
                if (alt < dist[edge.to]) {
                    dist[edge.to] = alt;
                    prev[edge.to] = { from: currentId as number, edge: edge };
                }
            });
        }
    ...
    function findNearestShelter(
        graph: { nodes: EvacNode[]; nodeMap: Record<string, number> },
        cityNodeId: number
    ): { shelterId: number | null; distancePx: number; path: EvacEdge[] | null } {
        const nodes = graph.nodes;
        let nearestShelterId: number | null = null;
        let minDist = Infinity;
        let shortestPath: EvacEdge[] | null = null;
    ...
    function calculateEvacuationPlan(
        cities: City[],
        shelters: Shelter[],
        roads: Road[],
        scale: number,
        ...
        roadDensities: Array<{ edge: EvacEdge; density: number; flow: number }>;
        ...
        const nodes = graph.nodes;
        const roadUsage: Record<string, { edge: EvacEdge; totalFlow: number; density: number }> = {};
        const cityPlans: Array<{
            cityIndex: number;
            population: number;
            shelterId: number | null;
            distanceKm: number;
        ...
        const roadDensities: Array<{ edge: EvacEdge; density: number; flow: number }> = [];
    ...
    function generateRoadNetwork(width: number, height: number, cities: City[], shelters: Shelter[]): Road[] {
        const roads: Road[] = [];
        const allNodes: Array<{ x: number; y: number; type: string; index: number }> = [];

...
const TERRAIN_FEATURE_TYPES: Record<string, string> = {
  MOUNTAIN: 'mountain',
  HILL: 'hill',
  BASIN: 'basin',
  PLAIN: 'plain'
...
const TERRAIN_PRESETS: Record<string, TerrainPreset> = {
...
const DAMAGE_LEVELS: Record<DamageLevel, DamageLevelInfo> = {
...
function getBuildingDamageLevel(buildingType: string, overpressurePsi: number): DamageLevel {
  const bt = BUILDING_TYPES[buildingType as BuildingTypeKey]
...
function calculateBuildingCasualties(
  ...
  const bt = BUILDING_TYPES[buildingType as BuildingTypeKey]
  if (!bt) return { deaths: 0, injured: 0, damageLevel: 'destroyed', buildingType, overpressure: overpressurePsi }
  ...
  const injuredRatio: Record<DamageLevel, number> = {
    intact: 0.02,
    light: 0.08,
    moderate: 0.2,
    severe: 0.35,
  ...
  const injuredRate = injuredRatio[damageLevel] || 0

  const totalInjured = population * injuredRate * (1 - totalDeaths / population)
...
function generateBuildingDistribution(isCentral: boolean, size: number): Record<string, number> {
...
function calculateCityBuildingDamage(
  ...
  explosions.forEach(function (exp) {
    ...
    zones.forEach(function (zone) {
      if (radii[zone.key] !== undefined && distKm <= radii[zone.key]) {
  ...
  const buildingResults: Record<string, {
    population: number
    damageLevel: DamageLevel
    deaths: number
    injured: number
  ...
  const distByDamage: Record<DamageLevel, number> = {
    intact: 0,
    light: 0,
    moderate: 0,
    severe: 0,
...
function calculateAvgStructureFactor(buildingDistribution: Record<string, number>): number {
  let total = 0
  let weightedSum = 0
  BUILDING_TYPE_ORDER.forEach(function (type) {
...
function calculateAllCitiesBuildingDamage(
  ...
  const totalByDamage: Record<DamageLevel, number> = {
    intact: 0,
    light: 0,
    moderate: 0,
    severe: 0,
  ...
  cities.forEach(function (city) {
    ...
    Object.keys(totalByDamage).forEach(function (level) {
      totalByDamage[level as DamageLevel] += result.distByDamage[level as DamageLevel] || 0
    })
...
function calculateCasualties(
  ...
  cities.forEach(function (city) {
    ...
    if (r && distKm <= r.fireball) {
      deaths += pop * 0.99
      city.destroyed = true
    } else if (r && distKm <= r.radiation) {
      deaths += pop * 0.85
      injured += pop * 0.1
      city.destroyed = true
    } else if (r && distKm <= r.severe) {
      deaths += pop * 0.5
      injured += pop * 0.4
      city.destroyed = true
    } else if (r && distKm <= r.moderate) {
      deaths += pop * 0.15
      injured += pop * 0.5
    } else if (r && distKm <= r.light) {
      deaths += pop * 0.02
      injured += pop * 0.2
    } else if (r && distKm <= r.thermal) {
      injured += pop * 0.05
    }
  })
...
function getElevationAt(x: number, y: number, terrain: TerrainData | null | undefined): number {
...
const ROAD_BASE_CAPACITY: number = 2000
const VEHICLE_SPEED_KMH: number = 60
const PEOPLE_PER_VEHICLE: number = 3
...
interface EvacuationPlanResult {
  graph: EvacuationGraph
  cityPlans: CityEvacuationPlan[]
  roadDensities: { edge: EvacuationEdge; density: number; flow: number }[]
...
interface DijkstraResult {
  ...
  prev: Record<number, { from: number; edge: EvacuationEdge } | null>
}

interface NearestShelterResult {
  shelterId: number | null
  distancePx: number
  path: EvacuationEdge[] | null
}
...
function dijkstra(graph: EvacuationGraph, sourceId: number): DijkstraResult {
...
function findNearestShelter(graph: EvacuationGraph, cityNodeId: number): NearestShelterResult {
  const nodes = graph.nodes
...
function calculateEvacuationPlan(
  ...
  const nodes = graph.nodes
  const roadUsage: Record<string, RoadUsage> = {}
  const cityPlans: CityEvacuationPlan[] = []
  ...
  const roadDensities: { edge: EvacuationEdge; density: number; flow: number }[] = []
...
const Physics = {
  ...
  calculateRadii,
  generateCities,
  generateBuildingDistribution,
  generateRoads,
  generateRoadNetwork,
  generateShelters,
  buildEvacuationGraph,
  dijkstra,
  findNearestShelter,
  ...
  calculateCombinedArea,
  calculateTotalAreaSum,
  calculateOverlapArea,
  calculateAllCombinedStats,
  calculateCombinedAreaPerZone,
  mulberry32
}

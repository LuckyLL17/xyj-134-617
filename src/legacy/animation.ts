...
export interface ExplosionAnimState {
  cx: number
  cy: number
  scale: number
...
export interface ShockwaveFront {
  km: number
  px: number
  fraction: number
}
...
function createExplosionAnimState(
  explosion: Explosion, index: number, scale: number, terrain: TerrainData | null): ExplosionAnimState {
  let cx = (explosion.explosionCenter as Point).x
  let cy = (explosion.explosionCenter as Point).y
  let radii = explosion.radii as Record<string, number>
  let safeScale = scale
  ...
  if (!radii || !isFinite(radii.fireball)) {
    ...
    radii = window.Physics.calculateRadii(yieldKt, burstH)
  }
...
function drawPersistentFireball(effectCtx: CanvasRenderingContext2D, a: ExplosionAnimState, progress: number): void {
  ...
  fbGrad.addColorStop(0, window.DataDisplay.rgba(255, 255, 220, fbAlpha))
  fbGrad.addColorStop(0.4, window.DataDisplay.rgba(255, 180, 80, fbAlpha * 0.8))
  fbGrad.addColorStop(0.7, window.DataDisplay.rgba(255, 100, 30, fbAlpha * 0.5))
  fbGrad.addColorStop(1, window.DataDisplay.rgba(255, 50, 0, 0))
  effectCtx.beginPath()
...
function drawShockwaveRing(
  ...
  function getRadiusAtAngle(angle: number): number {
    ...
    return window.Physics.calculateShockwaveRadiusAtAngle(
      a.explosionRef, angle, zoneKey, terrain as TerrainData, scale, currentSW
    )
  }
...
function drawPhaseFlash(effectCtx: CanvasRenderingContext2D, a: ExplosionAnimState, t: number): void {
  const p = t / 0.05
  ...
  if (currentFireballRadius > 0) {
    ...
    if (isFinite(cx) && isFinite(cy) && isFinite(r1)) {
      ...
      fbGrad.addColorStop(0, window.DataDisplay.rgba(255, 255, 255, 1))
      fbGrad.addColorStop(0.3, window.DataDisplay.rgba(255, 255, 200, 0.95))
      fbGrad.addColorStop(0.6, window.DataDisplay.rgba(255, 200, 100, 0.85))
      fbGrad.addColorStop(0.85, window.DataDisplay.rgba(255, 100, 0, 0.7))
      fbGrad.addColorStop(1, window.DataDisplay.rgba(255, 50, 0, 0))
      effectCtx.beginPath()
  ...
  if (flashAlpha > 0) {
    ...
    if (isFinite(cx) && isFinite(cy) && isFinite(r2)) {
      ...
      flashGrad.addColorStop(0, window.DataDisplay.rgba(255, 255, 255, flashAlpha * 0.8))
      flashGrad.addColorStop(0.5, window.DataDisplay.rgba(255, 255, 255, flashAlpha * 0.4))
      flashGrad.addColorStop(1, window.DataDisplay.rgba(255, 200, 100, 0))
      effectCtx.beginPath()
...
function drawPhaseFireball(effectCtx: CanvasRenderingContext2D, a: ExplosionAnimState, t: number): void {
  const p = t / 0.3
  ...
  if (p < 1) {
    ...
    fbGrad.addColorStop(0, window.DataDisplay.rgba(255, 255, 255, 0.95))
    fbGrad.addColorStop(0.4, window.DataDisplay.rgba(255, 240, 180, 0.9))
    fbGrad.addColorStop(0.7, window.DataDisplay.rgba(255, 180, 80, 0.75))
    fbGrad.addColorStop(1, window.DataDisplay.rgba(255, 80, 0, 0))
    effectCtx.beginPath()
  ...
  if (p > 0.3) {
    ...
    effectCtx.strokeStyle = window.DataDisplay.rgba(255, 220, 150, earlySWAlpha * 0.9)
    drawShockwaveRing(effectCtx, a, earlySW, earlySWAlpha, a.terrain, a.scale, 'fireball')

    for (let i = 0; i < 3; i++) {
      const innerSW = earlySW * (1 - (i + 1) * 0.2)
      if (innerSW > 0) {
        effectCtx.strokeStyle = window.DataDisplay.rgba(255, 150, 80, earlySWAlpha * 0.5)
...
function drawZoneProgress(
  ...
  const colors = window.Renderer.getZoneColors(zoneKey, a.tintIndex)
  const useTerrain = !!(a.terrain && a.terrain.features && a.terrain.features.length > 0)

  if (useTerrain) {
    const boundaryPoints = window.Physics.generateTerrainBoundaryPolygon(
      a.explosionRef, zoneKey, a.terrain as TerrainData, a.scale, 72
    )
    if (boundaryPoints && boundaryPoints.length > 0) {
      const clippedPoints = boundaryPoints.map(pt => {
        ...
        return {
          ...
          angle: pt.angle,
          attenuation: pt.attenuation
        }
      })

      window.Renderer.drawPolygonPathFromPoints(effectCtx, clippedPoints, true)
    ...
    effectCtx.fillStyle = window.DataDisplay.rgba(
      fillColor.r,
      fillColor.g,
      fillColor.b,
      alpha * fillColor.a
...
function drawRippleWave(
  ...
  function getRadiusAtAngle(angle: number, baseRadius: number): number {
    ...
    return window.Physics.calculateShockwaveRadiusAtAngle(
      a.explosionRef, angle, 'severe', a.terrain as TerrainData, a.scale, baseRadius
    )
  }
...
function drawPhaseShockwave(effectCtx: CanvasRenderingContext2D, a: ExplosionAnimState, t: number): void {
  ...
  a.particles.forEach(function (particle: Particle) {
    ...
    if (particleAlpha > 0) {
      ...
      effectCtx.fillStyle = window.DataDisplay.hexToRgba(particle.color, particleAlpha)
      effectCtx.fill()
    }
  })
...
function drawPhaseMushroom(effectCtx: CanvasRenderingContext2D, a: ExplosionAnimState, t: number): void {
  ...
  a.zoneOrder.forEach(zoneKey => {
    if (a.displayedZones[zoneKey]) {
      ...
      if (useTerrain) {
        ...
        if (boundaryPoints && boundaryPoints.length > 0) {
          window.Renderer.drawPolygonPathFromPoints(effectCtx, boundaryPoints, true)
          effectCtx.fillStyle = window.DataDisplay.rgba(
  ...
  stemGrad.addColorStop(0, window.DataDisplay.rgba(sr[0]*0.7, sr[1]*0.5, sr[2]*0.4, 0.8 * (1 - mp * 0.3)))
  stemGrad.addColorStop(1, window.DataDisplay.rgba(sr[0]*0.5, sr[1]*0.35, sr[2]*0.25, 0.4 * (1 - mp * 0.5)))
  ...
  cloudGrad.addColorStop(0, window.DataDisplay.rgba(sr[0], sr[1], sr[2], 0.85 * (1 - mp * 0.4)))
  cloudGrad.addColorStop(0.5, window.DataDisplay.rgba(sr[0]*0.75, sr[1]*0.55, sr[2]*0.35, 0.7 * (1 - mp * 0.5)))
  cloudGrad.addColorStop(1, window.DataDisplay.rgba(sr[0]*0.45, sr[1]*0.4, sr[2]*0.3, 0))
  ...
  if (mp > 0.3) {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      effectCtx.fillStyle = window.DataDisplay.rgba(sr[0]*0.6, sr[1]*0.5, sr[2]*0.4, debrisAlpha * (0.5 + (i * 0.013) % 0.5))
      effectCtx.fill()
    }
  }
  ...
  a.particles.forEach(function (particle: Particle) {
    ...
    if (particleAlpha > 0) {
      ...
      effectCtx.fillStyle = window.DataDisplay.hexToRgba(particle.color, particleAlpha * 0.3)
      effectCtx.fill()
    }
  })
}

function drawPhaseFadeOut(effectCtx: CanvasRenderingContext2D, a: ExplosionAnimState, t: number): void {
  ...
  cloudGrad.addColorStop(0, window.DataDisplay.rgba(sr[0]*0.8, sr[1]*0.6, sr[2]*0.4, 0.6 * fadeAlpha))
  cloudGrad.addColorStop(0.5, window.DataDisplay.rgba(sr[0]*0.6, sr[1]*0.45, sr[2]*0.3, 0.4 * fadeAlpha))
  cloudGrad.addColorStop(1, window.DataDisplay.rgba(sr[0]*0.35, sr[1]*0.3, sr[2]*0.25, 0))
  effectCtx.fillStyle = cloudGrad
  effectCtx.beginPath()
  for (let ang = 0; ang < 8; ang++) {
    const angle = (ang / 8) * Math.PI * 2
...
function triggerSounds(animStates: ExplosionAnimState[], tSeconds: number, intensity: number): void {
  const audioManager = window.AudioManager as any
  animStates.forEach(a => {
    ...
    if (!a.soundTriggers.explosion && tSeconds >= 0.05) {
      ...
      if (audioManager) {
        audioManager.playExplosionSound(intensity)
      }
    }
    ...
    if (!a.soundTriggers.shockwaveNear && tSeconds >= a.zoneDisplayTimes.severe * 0.5) {
      ...
      if (audioManager) {
        audioManager.playShockwaveSound(a.radii.severe * 0.3, intensity)
      }
    }
    ...
    if (!a.soundTriggers.shockwaveFar && tSeconds >= a.zoneDisplayTimes.moderate) {
      ...
      if (audioManager) {
        audioManager.playShockwaveSound(a.radii.moderate * 0.5, intensity * 0.8)
      }
    }
    ...
    if (!a.soundTriggers.quake && tSeconds >= a.quakeStartTime) {
      ...
      if (audioManager) {
        audioManager.playQuakeSound(intensity, 0)
      }
    }
    ...
    if (!a.soundTriggers.debris && tSeconds >= a.zoneDisplayTimes.moderate + 0.3) {
      ...
      if (audioManager) {
        audioManager.playDebrisSound(intensity)
      }
    }
  })
}
...
function animateExplosion(
  ...
  activeExplosions.forEach(function (e: Explosion) {
    ...
    if (!e.radii || !isFinite(e.radii.fireball)) {
      e.radii = window.Physics.calculateRadii(e.yieldKilotons, e.burstHeight || 1000)
    }
  })
  ...
  const animStates = activeExplosions.map(function (exp: Explosion, idx: number) {
  ...
  function draw(now: number): void {
    ...
    if (totalQuakeIntensity > 0.05) {
      applyEarthquakeShake(mapCtx, width, height, totalQuakeIntensity, tSeconds)
      window.Renderer.drawMap(mapCtx, mapWrapper, state)
      restoreFromEarthquake(mapCtx)
    } else {
      window.Renderer.drawMap(mapCtx, mapWrapper, state)
    }
    ...
    if (progress < 1) {
      ...
      setTimeout(function () {
        ...
        if (window.Timeline) {
          window.Timeline.start(state, effectCtx, effectCanvas)
        }
      }, 800)
    }
  }
...
export const Animation = {
  animateExplosion: animateExplosion as any
} as AnimationModule

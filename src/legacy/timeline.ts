import type { Explosion, City, TerrainData, Point, AppState, TimelineModule } from '@/types/index'
...
type ElementsMap = Record<string, HTMLElement | null>

const rgba = window.DataDisplay.rgba
const formatNumber = window.DataDisplay.formatNumber
...
export function startTimelineMode(
  state: AppState,
  ctx?: CanvasRenderingContext2D | null,
  canvas?: HTMLCanvasElement | null
): void {
  const activeExplosions = state.explosions.filter(function (e: Explosion): boolean {
    return e.explosionCenter !== null && e.radii !== null
  })
  ...
  const casualties = window.Physics.calculateCasualtiesTerrainAware(
...
function drawTrendLine(
  ...
  const points = data.map(function (value: number, idx: number): { x: number; y: number } {
...
export function getCurrentProgress(): number {
  return timelineState.currentProgress
}
...
export { TIMELINE_STAGES as STAGES }

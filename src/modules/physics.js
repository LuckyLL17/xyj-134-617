...
export const TERRAIN_FEATURE_TYPES = {
    MOUNTAIN: 'mountain',
    HILL: 'hill',
    BASIN: 'basin',
    PLAIN: 'plain'
...
export const BUILDING_TYPE_ORDER = ['temporary', 'wood', 'brick', 'rc_frame', 'steel', 'rc_core', 'blast_resistant'];
...
export function getBuildingDamageLevel(buildingType, overpressurePsi) {
...
export function getOverpressureAtDistance(yieldKilotons, distanceKm, burstHeight) {
...
export function calculateBuildingCasualties(population, buildingType, overpressurePsi, indoorRatio) {
    const bt = BUILDING_TYPES[buildingType];
...
export function getZoneKeys() {
    return ZONE_DEFS.map(function (z) { return z.key; });
}

export function getZonePriority() {
    return ZONE_DEFS.map(function (z) { return z.key; });
}

export function getZoneAltitudeSensitivity() {
    const result = {};
...
export function addZone(zoneDef) {
...
export function updateZone(key, updates) {
...
export function removeZone(key) {
    const zoneIndex = ZONE_DEFS.findIndex(function (z) { return z.key === key; });
    if (zoneIndex < 0) {
        return { success: false, error: '圈层不存在' };
    }
...
export function moveZone(key, newIndex) {
    const zoneIndex = ZONE_DEFS.findIndex(function (z) { return z.key === key; });
    if (zoneIndex < 0) {
        return { success: false, error: '圈层不存在' };
    }
...
export function calculateZoneRadius(zone, W_megatons, burstHeight) {
    try {
        const W = W_megatons;
...
export function generateBuildingDistribution(isCentral, size) {
    const dist = {};
...
export function generateCities(width, height) {
    const cities = [];
    const cityCount = 80;
...
export function generateRoads(width, height, cities) {
    const roads = [];
...
export function getWorstZoneForCity(city, explosions, scale) {
...
export function calculateCityBuildingDamage(city, explosions, scale, terrain) {
...
export function calculateAvgStructureFactor(buildingDistribution) {
    let total = 0;
    let weightedSum = 0;
    BUILDING_TYPE_ORDER.forEach(function (type) {
...
export function calculateAllCitiesBuildingDamage(cities, explosions, scale, terrain) {
    const results = [];
    let totalDeaths = 0;
    let totalInjured = 0;
    let totalDestroyedPop = 0;
...
export function calculateCasualties(cities, explosionCenter, radii, scale) {
    let deaths = 0;
    let injured = 0;
...
export function calculateCombinedCasualties(cities, explosions, scale) {
    let deaths = 0;
    let injured = 0;
...
export function calculateEnergy(yieldKilotons) {
    return Math.round(yieldKilotons * 4.184);
}

export function calculateTotalEnergy(explosions) {
    let total = 0;
    explosions.forEach(function (exp) {
...
export function calculateAffectedArea(radii) {
...
export function calculateCombinedAreaPerZone(explosions, scale, zoneName) {
    const circles = [];
    explosions.forEach(function (exp) {
        if (!exp.explosionCenter || !exp.radii) return;
...
export function calculateCombinedArea(explosions, scale) {
    return calculateCombinedAreaPerZone(explosions, scale, 'thermal');
}

export function calculateTotalAreaSum(explosions) {
    let total = 0;
    explosions.forEach(function (exp) {
        if (!exp.radii) return;
...
export function calculateOverlapArea(explosions, scale) {
...
export function calculateAllCombinedStats(explosions, scale) {
...
export function generateTerrainFeatures(width, height, presetName, elevationScale, seed) {
...
export function getElevationAt(x, y, terrain) {
...
export function calculatePathAttenuation(fromX, fromY, toX, toY, terrain, zoneName, scale, burstHeightMeters) {
    if (!terrain || !terrain.features || terrain.features.length === 0) {
...
export function calculateEffectiveRadius(explosion, targetX, targetY, zoneName, terrain, scale) {
...
export function generateTerrainBoundaryPolygon(explosion, zoneName, terrain, scale, segments) {
...
export function checkPointInAnyZoneTerrainAware(point, explosions, terrain, scale) {
    let worstZoneIndex = -1;
...
export function getWorstZoneForCityTerrain(city, explosions, scale, terrain) {
    if (!terrain || !terrain.features || terrain.features.length === 0) {
...
export function calculateCasualtiesTerrainAware(cities, explosions, scale, terrain) {
    let deaths = 0;
    let injured = 0;
...
export function calculateShockwaveRadiusAtAngle(explosion, angle, zoneKey, terrain, scale, baseRadiusPx) {
    if (!terrain || !terrain.features || terrain.features.length === 0) {
        return baseRadiusPx;
    }
...
export const ROAD_BASE_CAPACITY = 2000;
export let VEHICLE_SPEED_KMH = 60;
export const PEOPLE_PER_VEHICLE = 3;

export function setVehicleSpeed(speed) {
    VEHICLE_SPEED_KMH = speed;
}

export function buildEvacuationGraph(cities, shelters, roads, capacityMultiplier) {
...
export function findNearestShelter(graph, cityNodeId) {
    const nodes = graph.nodes;
...
export function calculateEvacuationPlan(cities, shelters, roads, scale, warningTimeMinutes, roadCapacityMultiplier, vehicleSpeedKmh) {
...
export function generateShelters(width, height, count) {
    const shelters = [];
    const n = count || 3;
    const centerX = width / 2;
    const centerY = height / 2;
...
export function generateRoadNetwork(width, height, cities, shelters) {
    const roads = [];
    const allNodes = [];
